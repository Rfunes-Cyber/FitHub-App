...
public class NutritionCalc {
    ...
    private static double getActivityMultiplier(String activityLevel) {
        return switch (activityLevel) {
            case "Little to none exercise" -> 1.20;
    ...
    private static double adjustCaloriesForGoal(
            double tdee,
            String fitnessGoal) {

        return switch (fitnessGoal) {
            case "Maintain Weight" -> tdee;
            case "Weight loss" -> tdee - 500;
            case "Gain Weight" -> tdee + 500;
            case "Extreme weight loss" -> tdee + 300;
            case "Extreme weight gain" -> tdee + 1000;
            default -> throw new IllegalArgumentException("Invalid fitness goal.");
        };
    }
